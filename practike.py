...
def min_stone_weight(A, B, C, D):
    ...
    # Проверяем случай, когда самый тяжелый сидит отдельно
    diff = weights[2] - (weights[0] + weights[1])
    if diff <= D:
        return 0
    
    # Проверяем случай, когда два самых тяжелых сидят вместе
    diff = (weights[1] + weights[2]) - weights[0]
    if diff <= D:
        return 0
    
    # Если ни один из случаев не подходит, вычисляем необходимый вес камня
    return diff - D
...
print(f"Минимальный вес камня: {result}")

# Дополнительный тестовый случай
A2, B2, C2, D2 = 30, 20, 45, 10
result2 = min_stone_weight(A2, B2, C2, D2)
print(f"Минимальный вес камня для второго случая: {result2}")
...
def min_stone_weight(A, B, C, D):
    weights = [A, B, C]
    min_stone = float('inf')  # Инициализируем минимальный вес камня как бесконечность
    
    # Рассматриваем все возможные комбинации
    for i in range(3):
        for j in range(i+1, 3):
            # Двое на одной стороне, один на другой
            diff = abs(weights[i] + weights[j] - weights[3-i-j])
            if diff <= D:
                return 0  # Если разница допустима, камень не нужен
            else:
                min_stone = min(min_stone, diff - D)
    
    return min_stone

# Ввод данных
A = int(input("Введите массу Ани: "))
B = int(input("Введите массу Бори: "))
C = int(input("Введите массу Саши: "))
D = int(input("Введите допустимую разницу: "))
